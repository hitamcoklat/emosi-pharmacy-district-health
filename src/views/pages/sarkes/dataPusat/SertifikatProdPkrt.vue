    ...
    <v-card color="basil" flat>
          ...
          <v-btn
            ...
            @click="
              $router.push({
                ...
                params: {
                  title: 'Import Data Sertifikat Produksi PKRT Pusat',
                  tableName: 'master_ser_prod_pkrt_pusat',
                },
              })
            "
            depressed
      ...
      <v-row>
        <v-col>
          ...
          <div v-if="tampilanExcellCabang">
            <ViewExcell :itemData="itemPusat" :showButtonExport="false" />
          </div>
        </v-col>
      </v-row>
    </v-card>
...
import ViewExcell from "@/components/excell/SertifikatProdPkrt"
...
export default {
  data() {
    return {
      ...
      headersPusat: [
        ...
        { text: "Alamat Gudang", value: "alamatPabrik", width: "200px" },
        { text: "Kab/Kota Gudang", value: "kabKota", width: "200px" },
        { text: "Telp. Kantor", value: "telpKantor", width: "200px" },
        { text: "No. Sertifikat", value: "nomorSertifikat", width: "200px" },
        { text: "NPWP", value: "npwp", width: "200px" },
        { text: "Jenis Produksi", value: "jenisProduksi", width: "300px" },
        { text: "NIB", value: "nib", width: "200px" },
        { text: "Jenis Permohonan", value: "jenisPermohonan", width: "200px" },
  ...
  methods: {
    ...
    updateTutup: function (item, i) {
      ...
      this.$http
        .post(
          this.$apiUrl + "/sarkes/update-tutup?t_name=verif_ser_prod_pkrt",
          { content: dataKirim }
        )
        .then((response) => {
    ...
    inputVerif: function () {
      ...
      this.$http
        .post(
          this.$apiUrl + "/sarkes/input-verifikasi?t_name=verif_ser_prod_pkrt",
          { content: data }
        )
        .then((response) => {
          if (response.data.status == true) {
    ...
    lihatDataVerifikasi: function () {
      ...
      this.$http
        .get(
          this.$apiUrl + "/sarkes/read-verifikasi?t_name=verif_ser_prod_pkrt"
        )
        .then((response) => {
    ...
    submitCapa: function () {
      ...
      this.$http
        .post(this.$apiUrl + "/sarkes/update-capa?t_name=verif_ser_prod_pkrt", {
          content: dataKirim,
        })
        .then((response) => {
          if (response.data.status == true) {
    ...
    removeData: function () {
      ...
      this.$http
        .post(
          this.$apiUrl +
            "/sarkes/remove-master?t_name=master_ser_prod_pkrt_pusat",
          { content: data }
        )
        .then((response) => {
          if (response.data.status == true) {
    ...
    removeItem: function (item) {
      let tblName = "master_ser_prod_pkrt_pusat";
      ...
      if (confirm("Apakah anda yakin ingin menghapus data ini?")) {
        this.$http
          .get(
            this.$apiUrl +
              "/sarkes/remove-verifikasi?t_name="+tblName+"&id=" +
              item.ID
          )
          .then((response) => {
            if (response.data.status == true) {
    ...
    updateStandar: function (item, i) {
      ...
      this.$http
        .post(
          this.$apiUrl + "/sarkes/update-standar?t_name=verif_ser_prod_pkrt",
          { content: dataKirim }
        )
        .then((response) => {
          if (response.data.status == true) {
    ...
    dataCabang: async function () {
      ...
      const res = await fetch(
        `${this.$apiUrl}/sarkes/read-template-pusat?tbl=master_ser_prod_pkrt_pusat`
      );
