    ...
    <v-content class="pl-0">
      <v-container>
        ...
        <div class="my-2">
          ...
          <v-row v-if="saveBtn" no-gutters>
            <v-btn @click="itemData = []; saveBtn = false" class="pb-7 pt-7 mr-2" color="primary"
              >Reset</v-btn
            >
...
export default {
  ...
  methods: {
    downloadTemplate: function () {
      window.open(`${this.$downloadTemplate}/template-sertifikat-produksi-alkes-pusat.xlsx`, "_blank");
    },
      ...
      readXlsxFile(input.files[0]).then((rows) => {
        ...
        for (var i = 1; i <= rows.length; i++) {
          try {
            console.log(rows[i]);
            dataItem = {
              nomorPerusahaan: rows[i][1],
              namaPerusahaan: rows[i][2],
              npwp: rows[i][3],
              nib: rows[i][4],
              alamatPerusahaan: rows[i][5],
              provinsi: rows[i][6],
              kabKota: rows[i][7],
              alamatPabrik: rows[i][8],
              provinsiPabrik: rows[i][9],
              kabkotaPabrik: rows[i][10],
              nomorSertifikat: rows[i][11],
              tanggalTerbit: rows[i][12],
              tanggalExpired: rows[i][13],
              kategoriYangDiProduksi: rows[i][14],
              jenisProduksi: rows[i][15],
              jenisPermohonan: rows[i][16],
              jenisPerubahan: rows[i][17],
              emailKantor: rows[i][18],
              telpKantor: rows[i][19],
            };
