...
export default {
  ...
  methods: {
    updateData: function () {
      ...
      this.$http
        .put(this.$apiUrl + "/sarkes/input-verifikasi?t_name=verifpak", {
          content: dataKirim,
        })
        .then((response) => {
    ...
    removeData: function () {
      this.$http
        .get(
          this.$apiUrl +
            "/sarkes/remove-verifikasi?t_name=verifpak&id=" +
            this.content.ID
        )
        .then((response) => {
          if (response.data.status == true) {
  ...
  created() {
    ...
    console.log("listKabkot", listKabkot);
  },
};
</script>
