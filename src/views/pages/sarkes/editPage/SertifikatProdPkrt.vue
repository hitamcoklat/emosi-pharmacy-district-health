    ...
    <v-content class="pl-0">
      <v-container>
        <p style="font-weight: bold; font-size: 1.3em" class="mt-n12 mx-auto">
          Edit Sertifikat Produksi P K R T
        </p>
        <v-row no-gutters>
...
export default {
  ...
  methods: {
    updateData: function () {
      ...
      this.$http
        .put(this.$apiUrl + "/sarkes/input-verifikasi?t_name=verif_ser_prod_pkrt", {
          content: dataKirim,
        })
        .then((response) => {
    ...
    removeData: function () {
      this.$http
        .get(
          this.$apiUrl +
            "/sarkes/remove-verifikasi?t_name=verif_ser_prod_pkrt&id=" +
            this.content.ID
        )
        .then((response) => {
          if (response.data.status == true) {
