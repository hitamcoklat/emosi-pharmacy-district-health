<template>
    <div>
        ...
        <v-content class="pl-0">
          <v-container>
              <p style="font-weight: bold; font-size: 1.3em;" class="mt-n12 mx-auto">Data Toko Alkes</p>
              ...
              <div>
                <vue-excel-editor v-model="itemData">
                    ...
                    <vue-excel-column field="tglnoizin"  label="Tgl. No Izin" />
                    ...
                    <vue-excel-column field="pend_pjawab"  label="pend_pjawab" />
                    <vue-excel-column field="sert_pjawab"  label="sert_pjawab" />
                    <vue-excel-column field="jproduk"  label="jproduk" />
                    <vue-excel-column field="ket"  label="Keterangan" />
  ...
  name: 'Oblik',
    ...
    methods: {
      ...
      downloadTemplate: function() {
        window.open('http://maubikin.web.id/e-mosi/public-assets/upload/template_upload_tokoalkes(kabkota).xlsx', '_blank')
      },
      ...
      uploadTemplate: function() {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          for(var i=2; i<=rows.length; i++) {
            try {
              console.log(rows[i])
              dataItem = { no: no, nama_kota: rows[i][2], nama: rows[i][3], alamat: rows[i][4], noizin: rows[i][5], tglnoizin: rows[i][6], telp: rows[i][7], namapemilik: rows[i][8], pjawab: rows[i][9], jk: rows[i][10], pend_pjawab: rows[i][11], sert_pjawab: rows[i][12], jproduk: rows[i][13], ket: rows[i][14], status_verifikasi_kota: rows[i][15] }
      ...
      fetchData: function () {
        ...
        this.dialogLoading = true
        this.$http.get(this.$apiUrl + '/sarkes/getAllData?jenis='+this.pilihJenis+'&thn=' + this.pilihTahun + '&bln='+indexBulan+'&id_daerah=' + this.idDaerah)
