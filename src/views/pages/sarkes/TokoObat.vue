  ...
  name: 'Oblik',
    ...
    methods: {
      ...
      downloadTemplate: function() {
        window.open('http://maubikin.web.id/e-mosi/public-assets/upload/template_upload_tokoobat(kabkota).xlsx', '_blank')
      },
          ...
          for(var i=2; i<=rows.length; i++) {
            try {
              ...
              dataItem = { 
                ...
                nama_kota: rows[i][2], 
                namatoko: rows[i][3], 
                no_izin_toko_obat: rows[i][4], 
                tanggal_no_izin: rows[i][5], 
                alamat: rows[i][6], 
                telp: rows[i][7], 
                penanggungjawab: rows[i][8], 
                jk: rows[i][9], 
                sikpenanggungjawab: rows[i][10], 
                status_verifikasi_kota: rows[i][11] 
              }
      ...
      fetchData: function () {
        ...
        this.dialogLoading = true
        this.$http.get(this.$apiUrl + '/sarkes/getAllData?jenis=tokoobat&thn=' + this.pilihTahun + '&bln='+indexBulan+'&id_daerah=' + this.idDaerah)
