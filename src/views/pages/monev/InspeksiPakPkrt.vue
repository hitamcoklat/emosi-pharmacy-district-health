<template>
    <div>
        ...
        <v-content class="pl-0">
          <p style="font-weight: bold; font-size: 1.3em; margin-left: 10px;" class="mt-n12">Monev Inspeksi PAK PKRT</p>
          <v-col cols="12" sm="12" md="12">
              ...
              <date-range-picker
                  ...
                  <v-btn style="margin-left: 10px;" @click="btnPilihTanggal" color="primary" medium><v-icon style="margin-right: 10px;" size="16">mdi-calendar-range</v-icon> Pilih Tanggal</v-btn>
                  <v-btn style="margin-left: 10px;" @click="dialogFilterTahun = true;" color="success" medium><v-icon style="margin-right: 10px;" size="16">mdi-calendar-range</v-icon> Tampilkan Per Tahun</v-btn>
                ...
                <v-container style="margin: 10px;">
                  <v-row>
                    <v-col
                      cols="12"
                    >
                      <iframe style="width: 100%; height: 500px; border: 0 !important; background-color: grey;" :src="`${this.$linkMonev}/#/monev/inspeksi-pak-pkrt/32/${longlat}`" />
                    </v-col>
                  </v-row>
                </v-container>
              </v-tab-item>
...
import MapMonev from './map/pakpkrt'
import PopPbfExcell from './popExcell/pakpkrt'
...
export default {
        ...
        headers: [
          {
            ...
            align: 'start',
            sortable: false,
            value: 'CONTENT.content.page_2_1',
          },
          { text: 'Status Perusahaan', value: 'CONTENT.content.page_2_2' },
          { text: 'Alamat / Telp.', value: 'CONTENT.content.page_2_3' },
          { text: 'Nama Pimpinan', value: 'CONTENT.content.page_2_6' },
          { text: 'N P W P', value: 'CONTENT.content.page_2_8' },
    ...
    methods: {
      ...
      btnPilihTanggal: function() {
        this.$http.get(this.$apiUrl + '/filterDataMonev?jenis=monev_inspeksi_pak_pkrt&from=' + this.convertDate(this.dateRange.startDate) + '&to=' + this.convertDate(this.dateRange.endDate))
          .then((response) => {
      ...
      editItem: function(item) {
        ...
        this.paramEdit = this.$linkMonev + '/#/monev/inspeksi-pak-pkrt/32/' + item.LONGLAT + '?edit=true&id=' + item.id;
      ...
      deleteItem: function(item) {
        ...
        if(confirm('Apakah anda yakin ingin menghapus data ini?')) {
          this.$http.get(this.$apiUrl + '/remove-monev?id=' + item.id + '&jenis=monev_inspeksi_pak_pkrt')
            .then((response) => {
      ...
      btnShowAll: function() {
        this.$http.get(this.$apiUrl + '/getDataMonev?jenis=monev_inspeksi_pak_pkrt&thn=' + this.pilihFilterThn)
          .then((response) => {
            this.dataMap = response.data.data
          }) 
      },
