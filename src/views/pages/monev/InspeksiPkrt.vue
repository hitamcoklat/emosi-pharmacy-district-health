<template>
    <div>
        ...
        <v-content class="pl-0">
          <p style="font-weight: bold; font-size: 1.3em; margin-left: 10px;" class="mt-n12">Monev Inspeksi PKRT</p>
                ...
                <v-container style="margin: 10px;">
                  <v-row>
                    <v-col
                      cols="12"
                    >
                      <iframe style="width: 100%; height: 500px; border: 0 !important; background-color: grey;" :src="`${this.$linkMonev}/#/monev/inspeksi-pkrt/32/${longlat}`" />
                    </v-col>
                  </v-row>
                </v-container>
              </v-tab-item>
...
export default {
    ...
    methods: {
      ...
      btnPilihTanggal: function() {
        this.$http.get(this.$apiUrl + '/filterDataMonev?jenis=monevpkrt&from=' + this.convertDate(this.dateRange.startDate) + '&to=' + this.convertDate(this.dateRange.endDate))
          .then((response) => {
      ...
      editItem: function(item) {
        ...
        this.paramEdit = this.$linkMonev + '/#/monev/inspeksi-pkrt/32/' + item.LONGLAT + '?edit=true&id=' + item.id;
      ...
      deleteItem: function(item) {
        ...
        if(confirm('Apakah anda yakin ingin menghapus data ini?')) {
          this.$http.get(this.$apiUrl + '/remove-monev?id=' + item.id + '&jenis=monevpkrt')
            .then((response) => {
      ...
      btnShowAll: function() {
        this.$http.get(this.$apiUrl + '/getDataMonev?jenis=monevpkrt&thn=' + this.pilihFilterThn)
          .then((response) => {
            this.dataMap = response.data.data
          }) 
      },
