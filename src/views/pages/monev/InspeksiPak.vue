<template>
    <div>
        ...
        <v-content class="pl-0">
          <p style="font-weight: bold; font-size: 1.3em; margin-left: 10px;" class="mt-n12">Monev Inspeksi PAK</p>
                ...
                <v-container style="margin: 10px;">
                  <v-row>
                    <v-col
                      cols="12"
                    >
                      <iframe style="width: 100%; height: 500px; border: 0 !important; background-color: grey;" :src="`${this.$linkMonev}/#/monev/input-pak/32/${longlat}`" />
                    </v-col>
                  </v-row>
                </v-container>
              </v-tab-item>
      ...
      <!-- Loading Login -->    
      <PopExcell :dialogExcell=dialogExcell @dialog="tutupDialog" :dataMap="dataMap" />
      <v-dialog
        v-model="dialogEdit"
        persistent
        width="500">
...
import MapMonev from './map/pak'
import PopExcell from './popExcell/pak'
...
export default {
        ...
        headers: [
          {
            ...
            align: 'start',
            sortable: false,
            value: 'CONTENT.content.nama_perusahaan',
          },
          { text: 'Status Perusahaan', value: 'CONTENT.content.status_perusahaan' },
          { text: 'Alamat / Telp.', value: 'CONTENT.content.alamat_kantor_dan_telpfax' },
          { text: 'Nama Pimpinan', value: 'CONTENT.content.nama_pimpinan' },
          { text: 'N P W P', value: 'CONTENT.content.npwp' },
    ...
    methods: {
      ...
      btnPilihTanggal: function() {
        this.$http.get(this.$apiUrl + '/filterDataMonev?jenis=monevpak&from=' + this.convertDate(this.dateRange.startDate) + '&to=' + this.convertDate(this.dateRange.endDate))
          .then((response) => {
      ...
      editItem: function(item) {
        ...
        this.paramEdit = this.$linkMonev + '/#/monev/input-pak/32/' + item.LONGLAT + '?edit=true&id=' + item.id;
      ...
      btnShowAll: function() {
        this.$http.get(this.$apiUrl + '/getDataMonev?jenis=monevpak&thn=' + this.pilihFilterThn)
          .then((response) => {
            this.dataMap = response.data.data
          }) 
      },
